...
<div class="container-fluid">
    ...
    <div class="container-fluid" style="display: flex; justify-content: center; padding: 100px 0;">
        <div class="card border-secondary mb-3" style="max-width: 40rem; ">
            <div class="card-header" style="background-color: #143153; color: #ffffff;">ACTIVAR CUENTA</div>
            <div class="card-body text-secondary">
                <form id="restoreAccount" method="POST" action="{{route('update.account')}}">
                    @method('PUT')
                    @csrf
                    <div class="row ">
                        <div class="col-lg-12 py-3" style="width: 400px !important;">
                            <label for="password">NUEVA CONTRASEÑA:</label>
                            <input type="password" class="form-control" placeholder="CONTRASEÑA" name="password" id="password" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*_=+-]).{8,20}$" title="La contraseña debe tener más de 8 caracteres, una mayúscula, un número y un caracter especial " required>
                            <input type="hidden" value="{{$client_number}}" name="client_number">
                        </div>
                    </div>
                    <div class="modal-footer border-top-0">
                        <input type="submit" class="btn btn" style="background-color: #00A1E3;color: white;" id="sendNewPass" value="ACTIVAR CUENTA">
                    </div>
                </form>
            </div>
        </div>
