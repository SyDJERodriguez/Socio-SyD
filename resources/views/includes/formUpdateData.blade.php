          ...
          @if (isset($data))
              <form autocomplete="off" id="updateDataForm" method="POST" action="{{route('customer.updateData')}}">
                ...
                <div class="row ">
                      ...
                      <div class="col-lg-6 py-2" style="display: flex">
                          <input autocomplete="new-password" type="password" class="form-control btnBorder" name="password" id="password" placeholder="CONTRASEÑA" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[-¡!$%^&*()_+|~=`{}\[\]:@;'<>¿?,.\/]).{8,20}$" title="La contraseña debe tener más de 8 caracteres, una mayúscula, un número y un caracter especial "required>
                          <p style="color: red; margin: 0;">*</p>
                      </div>
