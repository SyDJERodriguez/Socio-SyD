          ...
          @if (isset($data))
              <form autocomplete="off" id="updateDataForm" method="POST" action="{{route('customer.updateData')}}">
                ...
                <div class="row ">
                      ...
                      <div class="col-lg-6 py-2" style="display: flex">
                          <input autocomplete="new-password" type="password" class="form-control btnBorder" placeholder="CONTRASEÑA" pattern="^(?=\w*\d)(?=\w*[A-Z])(?=\w*[a-z])(?=\w*[!#$%&'()*+,\-./:;<=>?@[\\\]^_`{|}~])\S{9,20}$" title="La contraseña debe tener más de 8 caracteres, una mayúscula, un número y un caracter especial " name="password" id="password" required>
                          <p style="color: red; margin: 0;">*</p>
                      </div>
