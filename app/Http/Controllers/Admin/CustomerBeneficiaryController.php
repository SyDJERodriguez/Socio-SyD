...
class CustomerBeneficiaryController extends Controller
{
    ...
    $insertBeneficiary = DB::table('beneficiaries')->insert([
                ...
                'customer_id'      => $request['id'][0],
                'branch_number'    => $request['branch_number'][0]
    ]);

    $insertBeneficiary2 = DB::table('beneficiaries')->insert([
        'name'             => $request['name'][1],
        'last_name'        => $request['lastname'][1],
        'second_last_name' => $request['second_lastname'][1],
        'relationship'     => $request['parent'][1],
        'mobile_number'    => $request['phone'][1],
        'percent'          => $request['percent'][1],
        'customer_id'      => $request['id'][0],
        'branch_number'    => $request['branch_number'][0]
]);
            ...
            $beneficiaries = DB::table('beneficiaries')
                ->where('customer_id','=', $request['id'][0])
                ->get();
            $beneficiaries = json_decode($beneficiaries);
            $beneficiary = (array)$beneficiaries;
         
     return view('Admin.customerBeneficiary', compact('beneficiary','data'));
