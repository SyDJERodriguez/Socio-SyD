...
class EmailsMonthly20 extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($registered_clients as $client){
            ...
            $level = '';
            $messsage_one   = 'Aun estas a tiempo de alcanzar el monto minimo de compra en este mes, para obtener tu seguro de accidentes personales de Socio SYD.';
            $messsage_two   = 'Este mes no has alcanzado el monto mínimo de compra para obtener los beneficios de asistencia Nivel Plata de Socio SYD.';
            $messsage_three = 'Este mes no has alcanzado el monto mínimo de compra para obtener los beneficios de asistencia Nivel Oro de Socio SYD.';
            if($client->type_user === '1'){
                if ($totalAmount<2500) {
                    TwilioService::send_sms($messsage_one,'+52'.$client->phone);
                ...
                if ($totalAmount>=2500 && $totalAmount<=4500) {
                    TwilioService::send_sms($messsage_two,'+52'.$client->phone);
                ...
                if ($totalAmount>4500 && $totalAmount<=7500) {
                    TwilioService::send_sms($messsage_three,'+52'.$client->phone);
