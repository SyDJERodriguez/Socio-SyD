...
class InsuranceInvitationToPlata30Negocio extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'schedule:invitationPlataNegocio30';
     ...
     * @var string
     */
    protected $description = 'Envio de invitacion negocios a plata cada 30 de Mes';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $destinataries = DB::table('customers_sessions')
                                ...
                                ->havingRaw('SUM(transactions.amount) < ?', [4500])
                                ->get();
                                //sin seguro negocios
            ...
            if(empty($destinataries) == false){
                ...
                foreach ($destinataries as $recipient) {
                    Mail::send('emails.seguroInvitacionPlataDuenio30Mes', [] ,function($m) use ($recipient) {
        ...
        $this->info('Envio de invitacion negocio a plata cada 30 de Mes');
    }
}
